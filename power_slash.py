...
class PowerCommands(commands.Cog):
    ...
    async def powerenter(
        ...
        player: str, tank: str, rocket: str, air: str, team4: str | None = None
    ):
        df=_df()
        new = {
            "player": player,
    ...
    async def powerplayervsplayer(
        ...
        diff=v1-v2; winner=player1 if diff>0 else player2 if diff<0 else "Remíza"
    ...
    class PlayerSelectView(discord.ui.View):
        ...
        def __init__(self, bot: commands.Bot, teams: int, players: list[str]):
            ...
            self.sub_candidates  = players[20:]
        ...
        def _mk_select(self, opts, placeholder, cb):
            select = discord.ui.Select(
                placeholder=placeholder,
                min_values=0,
                max_values=min(20,len(opts)),
                options=[discord.SelectOption(label=p) for p in opts],
            )
            select.callback = cb
            return select

        async def main_sel(self, inter: Interaction):
            self.selected_main = self.children[0].values
            await inter.response.defer()

        async def to_subs(self, inter: Interaction):
            self.clear_items()
            subs_opts = [p for p in self.sub_candidates if p not in self.selected_main]
            self.add_item(self._mk_select(subs_opts,"Pick additional players",self.sub_sel))
            done = discord.ui.Button(label="Done", style=discord.ButtonStyle.success)
            done.callback = self.finish; self.add_item(done)
            await inter.response.edit_message(view=self)

        async def sub_sel(self, inter: Interaction):
            self.selected_subs = self.children[0].values
            await inter.response.defer()
