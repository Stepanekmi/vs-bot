...
def safe_send_ephemeral(interaction: Interaction, msg: str):
    # Helper: interaction may already be responded/deferred
    try:
...
class PowerCommands(commands.Cog):
    ...
    async def powerplayer(self, interaction: Interaction, player: str):
        ...
        for team in ["tank", "rocket", "air"]:
            ...
            total_delta = 100 * (values[-1] - values[0]) / values[0] if len(values) > 1 and values[0] > 0 else 0.0
    ...
    async def powerplayervsplayer(self, interaction: Interaction,
        ...
        val1 = p1.iloc[0][team]; val2 = p2.iloc[0][team]
        diff = abs(val1 - val2)
    ...
    class PlayerSelectView(discord.ui.View):
        ...
        async def safe_wrap(self, func, interaction: Interaction, *a, **kw):
            try:
                ...
                await safe_send_ephemeral(interaction, "⚠️ Něco se pokazilo, zkuste to znovu.")
        ...
        async def main_selected(self, interaction: Interaction):
            try:
                ...
                logging.exception("main_selected error")
                await safe_send_ephemeral(interaction, "⚠️ Chyba při výběru.")
        ...
        async def to_subs(self, interaction: Interaction):
            ...
            self.select_subs = discord.ui.Select(
                placeholder="Pick substitutes (optional)",
                min_values=0,
                max_values=min(10, len(remaining)),
                options=opts,
            )
            ...
            async def subs_cb(sub_inter):
                try:
                    ...
                    logging.exception("subs_cb error")
                    await safe_send_ephemeral(sub_inter, "⚠️ Chyba při výběru subs.")
        ...
        async def finish(self, interaction: Interaction):
            ...
            try:
                ...
                await interaction.followup.send("⚠️ Něco se pokazilo při výpočtu týmů.", ephemeral=True)
            finally:
                self.stop()
        ...
        async def callback(self, interaction: Interaction):
            ...
            try:
                ...
                if scope == "all":
                    ...
                    df = df.drop(idx)
                # save CSV in executor with kwargs
