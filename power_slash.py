# power_slash.py – verze 2025-08-05
# Přináší všechny power příkazy + stormsetup + info
...
def _icon(t): return {"tank":"🛡️","rocket":"🚀","air":"✈️","team4":"⚙️"}.get(t,"•")
def _h(txt):  return f"**__{txt}__**"
...
class PowerCommands(commands.Cog):
    ...
    async def powerenter(self, inter:Interaction, player:str, tank:str, rocket:str, air:str, team4:str|None=None):
        df=_df(); new={"player":player,"tank":norm(tank),"rocket":norm(rocket),"air":norm(air),"timestamp":datetime.utcnow().isoformat()}
        if team4: new["team4"]=norm(team4)
        pd.concat([df,pd.DataFrame([new])],ignore_index=True).to_csv(POWER_FILE,index=False)
    ...
    async def powerplayer(self, inter:Interaction, player:str):
        ...
        dfp=_df(); dfp=dfp[dfp["player"].str.lower()==player.lower()].sort_values("timestamp")
        if dfp.empty: return await inter.followup.send("⚠️ Hráč nenalezen.",ephemeral=True)
        lines=[_h(player)]
        ...
        for c in ["tank","rocket","air"]: plt.plot(dfp["timestamp"],dfp[c],marker="o",label=c)
    ...
    async def powertopplayer(self, inter:Interaction):
        ...
        top_max=last.sort_values("max_team",ascending=False); top_tot=last.sort_values("total",ascending=False)
    ...
    async def powerplayervsplayer(self, inter:Interaction, player1:str, player2:str, team:str):
        ...
        last=_df().sort_values("timestamp").groupby("player",as_index=False).last()
        p1=last[last["player"].str.lower()==player1.lower()]; p2=last[last["player"].str.lower()==player2.lower()]
        if p1.empty or p2.empty: return await inter.response.send_message("Hráč nenalezen.",ephemeral=True)
        ...
        df=_df(); df1=df[df["player"].str.lower()==player1.lower()].sort_values("timestamp"); df2=df[df["player"].str.lower()==player2.lower()].sort_values("timestamp")
        plt.figure(figsize=(8,4)); plt.plot(df1["timestamp"],df1[team],marker="o",label=player1); plt.plot(df2["timestamp"],df2[team],marker="o",label=player2)
        plt.legend(); plt.tight_layout(); buf=io.BytesIO(); plt.savefig(buf,format="png"); buf.seek(0); plt.close()
    ...
    class PlayerSelectView(discord.ui.View):
        def __init__(self, bot:commands.Bot, teams:int, players:list[str]):
            ...
            self.selected_main,self.selected_subs=[],[]
            self.main_candidates=players[:20]; self.sub_candidates=players[20:]
            self.add_item(self._sel(self.main_candidates,"Pick main players (max 20)",self.main_sel))
            nxt=discord.ui.Button(label="Next", style=discord.ButtonStyle.primary); nxt.callback=self.to_subs; self.add_item(nxt)
        def _sel(self,opts,ph,cb):
            s=discord.ui.Select(placeholder=ph,min_values=0,max_values=min(20,len(opts)),options=[discord.SelectOption(label=p) for p in opts]); s.callback=cb; return s
        ...
        async def to_subs(self, inter):
            ...
            self.add_item(self._sel(subs,"Pick additional players",self.sub_sel))
