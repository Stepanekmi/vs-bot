...
async def setup_power_commands(bot: commands.Bot):
    ...
    @bot.tree.command(name="powertopplayer", description="Žebříček (3 týmy)")
    ...
    @bot.tree.command(name="powererase", description="Vymazat záznamy (last|all)")
    ...
    @bot.tree.command(name="powerlist", description="Vypsat záznamy hráče")
    ...
    @bot.tree.command(name="stormsetup", description="Vytvořit vyvážené týmy")
    async def stormsetup(interaction: discord.Interaction, teams: int):
        await interaction.response.defer()
        await interaction.followup.send(setup_storm(teams))

    @bot.tree.command(name="info", description="Nápověda k Power příkazům")
