...
def setup_vs_commands(bot: commands.Bot):
    ...
    async def vs_train(interaction: discord.Interaction):
        df = pd.read_csv(DB_FILE)
        # assume load_r4_list loaded globally
        latest = df["date"].max()
        df_day = df[df["date"] == latest]
        top = df_day.groupby("name")["points"].sum().reset_index().sort_values(by="points", ascending=False).head(1)
    ...
    async def info(interaction: discord.Interaction):
        help_text = (
            ...
            "/vs_train – send top TRAIN player\n"
            "/vs_r4 <tag> – top 2 R4 players\n"
